...
function commitElement(element, parentDom, path, root, prevInstances, nextInstances) {
  ...
  if (type === TEXT_ELEMENT) {
    const textNode = parentDom.ownerDocument.createTextNode(props.nodeValue ?? '');
  ...
  const dom = parentDom.ownerDocument.createElement(type);
...
function performRender(root) {
  ...
  while (root.container.firstChild) {
    root.container.removeChild(root.container.firstChild);
  }
